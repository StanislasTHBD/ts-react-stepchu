    ...
    <div className={`grid ${questionSecuritys.length === 0 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3'} gap-4 mt-4`}>
      {questionSecuritys.length === 0 ? (
        <div className="bg-white shadow-md rounded-lg p-4">
          <p className="text-lg font-semibold text-center">Aucun question sécurité</p>
        </div>
      ) : (
        questionSecuritys.map((questionSecurity) => (
          <div key={questionSecurity.id} className="bg-white shadow-md rounded-lg p-4">
            <p className="text-lg font-semibold">{questionSecurity.name}</p>
